...
public partial class AIComponent : Node
{
    ...
    public override void _Ready()
    {
        ...
        if (_owner is Actor actor)
        {
            _velocityComponent = actor.GetComponent(typeof(VelocityComponent)) as VelocityComponent;
        }
        else
        {
            _velocityComponent = _owner.GetNodeOrNull<VelocityComponent>("VelocityComponent");
        }
        ...
        if (NavigationAgent == null)
            NavigationAgent = _owner.GetNodeOrNull<NavigationAgent3D>("NavigationAgent3D");

        if (NavigationAgent == null)
            GD.PrintErr($"{_owner.Name}: No NavigationAgent3D found! AI needs one for movement.");
        ...
        {
            HealthComponent = actorWithHealth.GetComponent(typeof(HealthComponent)) as HealthComponent;
            if (HealthComponent != null)
                HealthComponent.Died += OnDied;
        }
    }
    ...
    public override void _PhysicsProcess(double delta)
    {
        ...
        switch (CurrentState)
        {
            ...
            case AIState.Dead:   /* handled above */          break;
        }
    }
    ...
    private void UpdateChase(double delta, float distance)
    {
        ...
        Vector3 lookTarget = new(_player.GlobalPosition.X, _owner.GlobalPosition.Y, _player.GlobalPosition.Z);
        ...
        if (_velocityComponent != null)
        {
            Vector3 direction = (_player.GlobalPosition - _owner.GlobalPosition).Normalized();
            direction.Y = 0;
            _velocityComponent.AccelerateInDirection(direction);
            _velocityComponent.ApplyGravity(_owner.GetGravity() * (float)delta);
