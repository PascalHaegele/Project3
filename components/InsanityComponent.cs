...
public partial class InsanityComponent : Node
{
    ...
    [Signal]
    public delegate void InsanityChangedEventHandler(float insanity);
    ...
    public void AddInsanity(float amount)
    {
        ...
        EmitSignal(SignalName.InsanityChanged, CurrentInsanity);
    ...
    private void UpdateInsanityLevel()
    {
        ...
        if (CurrentInsanity < MediumThreshold)
        {
            CurrentLevel = InsanityLevel.Normal;
        }
        else if (CurrentInsanity < HighThreshold)
        {
            CurrentLevel = InsanityLevel.Medium;
        }
        else
        {
            CurrentLevel = InsanityLevel.High;
        }
        ...
        if (previousLevel != CurrentLevel)
        {
            ...
            EmitSignal(SignalName.InsanityLevelChanged, (int)CurrentLevel);
